...
use App\Http\Requests\Admin\Solution\CreateSolutionParentCategoryRequest;
...
class ParentCategoryController extends Controller
{
    ...
    {
        $this->middleware('permission:solution-parent-category-index,admin');
        $this->middleware('permission:solution-parent-category-store,admin')->only('store');
        $this->middleware('permission:solution-parent-category-update,admin')->only(['edit','update','updateStatus']);
        $this->middleware('permission:solution-parent-category-delete,admin')->only('destroy');
    }
    /**
    ...
    {
        $parent_categories = SolutionParentCategory::with('admin')->where('parent_category_delete', 0)->get();
        return view('backend.blade.solution.parent_category.index', compact('parent_categories'));
    }
     ...
     */
    public function store(CreateSolutionParentCategoryRequest $data): Response
    {
    ...
    {
        $parent_category = SolutionParentCategory::findOrFail($id);
    ...
    public function updateStatus(Request $data){
        $parent_category = SolutionParentCategory::findOrFail($data->id);
