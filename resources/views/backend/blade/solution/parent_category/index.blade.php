...
@push('title')
    {{ __('admin_local.Parent Category') }}
@endpush
@push('css')
...
@section('content')
    <div class="container-fluid">
        <div class="page-title">
            <div class="row">
                <div class="col-sm-6">
                    <h3>{{ __('admin_local.Solution Parent Category List') }}</h3>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">{{ __('admin_local.Solution Parent Category List') }}</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="modal-content">
                <div class="modal-header d-flex align-items-center" style="border-bottom:1px dashed gray">
                    <h4 class="modal-title" id="myLargeModalLabel">
                        {{ __('admin_local.Add Solution Parent Category') }}
                    </h4>
    ...
    <div class="modal fade" id="edit-parent-category-modal" tabindex="-1" aria-labelledby="bs-example-modal-lg"
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header d-flex align-items-center" style="border-bottom:1px dashed gray">
                    <h4 class="modal-title" id="myLargeModalLabel">
                        {{ __('admin_local.Edit Solution Parent Category') }}
                    </h4>
                    <button type="button" class="btn-close " data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <p class="px-3 text-danger"><i>{{ __('admin_local.The field labels marked with * are required input fields.') }}</i>
                ...
                <div class="modal-body" style="margin-top: -20px">
                    <form id="edit_parent_category_form" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-6 mt-2">
                                <label for="parent_category_image"><strong>{{ __('admin_local.Previous Parent Category Image') }}
                                    </strong></label>
            ...
            <div class="col-lg-10 mx-auto">
                <div class="card">
                    <div class="card-header py-3" style="border-bottom: 2px dashed gray">
                        <h3 class="card-title mb-0 text-center">{{ __('admin_local.Solution Parent Category List') }}</h3>
                    </div>
                            ...
                            <div class="row mb-3">
                                <div class="col-md-3">
                                    <button class="btn btn-success" type="btn" data-bs-toggle="modal"
                                        data-bs-target="#add-parent-category-modal">+ {{ __('admin_local.Add Solution Parent Category') }}</button>
                                </div>
                            </div>
                        @endif
                                        ...
                                        <tr id="trid-{{ $parent_category->id }}"
                                            ...
                                            <td>
                                                @if ($parent_category->parent_category_image)
                                                    <img src="{{ asset(env('ASSET_DIRECTORY').'/'.$parent_category->parent_category_image) }}"
                                                        alt="" style="height:">
                                                @else
                                                    No File
                                                @endif
...
@push('js')
    ...
    <script>
        ...
        var form_url = "{{ route('admin.solution.parent-category.store') }}";
    ...
    <script src="{{ asset(env('ASSET_DIRECTORY').'/'.'admin/custom/solution/parent_category.js') }}"></script>
