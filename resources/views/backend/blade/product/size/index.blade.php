...
@section('content')
    <div class="container-fluid">
        <div class="page-title">
            <div class="row">
                <div class="col-sm-6">
                    <h3>{{ __('admin_local.Size List') }}</h3>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">{{ __('admin_local.Size') }}</li>
                    </ol>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="add-size-modal" tabindex="-1" aria-labelledby="bs-example-modal-lg" aria-hidden="true">
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <div class="modal-header d-flex align-items-center" style="border-bottom:1px dashed gray">
                    <h4 class="modal-title" id="myLargeModalLabel">
                        {{ __('admin_local.Add Size') }}
                    </h4>
                ...
                <div class="modal-body" style="margin-top: -20px">
                    <form method="POST" action="" id="add_size_form" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-lg-12 mt-2">
                                <label for="size_name"><strong>{{ __('admin_local.Size Name') }} *</strong></label>
    ...
    <div class="modal fade" id="edit-size-modal" tabindex="-1" aria-labelledby="bs-example-modal-lg" aria-hidden="true">
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <div class="modal-header d-flex align-items-center" style="border-bottom:1px dashed gray">
                    <h4 class="modal-title" id="myLargeModalLabel">
                        {{ __('admin_local.Edit Size') }}
                    </h4>
                    <button type="button" class="btn-close " data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <p class="px-3 text-danger"><i>{{ __('admin_local.The field labels marked with * are required input fields.') }}</i>
                ...
                <div class="modal-body" style="margin-top: -20px">
                    <form id="edit_size_form" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
            ...
            <div class="col-lg-10 mx-auto">
                <div class="card">
                    <div class="card-header py-3" style="border-bottom: 2px dashed gray">
                        <h3 class="card-title mb-0 text-center">{{ __('admin_local.Size List') }}</h3>
                    </div>

                    <div class="card-body">
                        @if (hasPermission(['size-store']))
                            <div class="row mb-3">
                                <div class="col-md-3">
                                    <button class="btn btn-success" type="btn" data-bs-toggle="modal"
                                        data-bs-target="#add-size-modal">+ {{ __('admin_local.Add Size') }}</button>
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="table-responsive theme-scrollbar">
                            <table id="basic-1" class="display table-bordered">
                                <thead>
                                    <tr>
                                        <th>{{ __('admin_local.Size Name') }}</th>
                                ...
                                <tbody>
                                    @foreach ($sizes as $size)
                                        <tr id="trid-{{ $size->id }}" data-id="{{ $size->id }}">
                                            <td>{{ $size->size_name }}</td>
                                            <td class="text-center">
                                                @if (hasPermission(['size-update']))
                                                    <span
                                                        class="mx-2">{{ $size->size_status == 0 ? 'Inactive' : 'Active' }}</span><input
                                                        data-status="{{ $size->size_status == 0 ? 1 : 0 }}"
                                                        id="status_change" type="checkbox" data-toggle="switchery"
                                                        data-color="green" data-secondary-color="red" data-size="small"
                                                        {{ $size->size_status == 1 ? 'checked' : '' }} />
                                                @else
                                                    <span class="badge badge-danger">{{ __('admin_local.No Permission') }}</span>
                                                @endif
                                            </td>
                                            <td>
                                                @if (hasPermission(['size-update','size-delete']))
                                                <div class="dropdown">
                                                    <button
                                                    ...
                                                    <div class="dropdown-content">
                                                        ...
                                                        <a data-bs-toggle="modal" style="cursor: pointer;"
                                                            data-bs-target="#edit-size-modal" class="text-primary"
...
@push('js')
    ...
    <script>
        ...
        var form_url = "{{ route('admin.product.size.store') }}";
    ...
    <script src="{{ asset(env('ASSET_DIRECTORY').'/'.'admin/custom/product/size.js') }}"></script>
